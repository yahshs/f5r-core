...
import { requestPanelV2Refill } from "../smm/panelV2Adapter";
...
describe("customer compensation bot", () => {
  ...
  it("keeps one stable customer link, reports live status, and enforces the refill limit", async () => {
    ...
    const startCode = new URL(stableLink).searchParams.get("start");
    await request(app)
    ...
    expect(sentMessages.some((message) => message.includes("241770081") && message.includes("مكتمل"))).toBe(true);
